...
# 0.57.0
## Features Added
  - Custom fields (experimental): `CustomField.update` accepts `options`, to add values to an enum field. Existing values can't be removed.

# 0.56.0
## Breaking Changes
  - `roboto triggers` now speaks the event-driven trigger model, replacing the `causes`/`for_each`/single-action command set of the same name. A trigger subscribes to platform events (`--on`, repeatable) at a de-duplication grain (`--once-per`), or fires on a UTC cron expression (`--schedule`), and dispatches one or more typed targets given as JSON (`--targets-json`). The flags the old set took on `create` and `update` are gone and now fail the command rather than being ignored: `--action`, `--required-inputs`, `--additional-inputs`, `--parameter-value` and `--timeout` are expressed inside an `invoke_action` target spec, and `--required-tag`/`--required-metadata` are expressed as a condition through `--condition-json`. `roboto triggers search` is removed; `roboto triggers list` lists an org's triggers, and the trigger query API serves only triggers expressible in the older shape, so it would have hidden exactly the triggers this model adds. `roboto triggers get`, `delete`, `enable` and `disable` are unchanged. New in the set: `dry-run`, which explains gate by gate whether a trigger would fire for a given dataset, file, invocation, session or event annotation without running anything, and `sample`, which prints a realistic sample of an event type and the `{{ }}` paths a condition or target template may reference. Existing triggers keep working and are readable and editable through the new commands; a trigger created through the older API accepts only edits its shape can express and says what it cannot store.
...
## Features Added
  - A trigger is now a firing source, an optional condition, and one or more targets, in the new `roboto.domain.triggers` module, with the platform-event vocabulary in `roboto.domain.platform_events`. The firing source is either a subscription to platform events — file uploaded, ingested or metadata-updated; dataset created, metadata-updated, tag-added or upload-completed; invocation completed or failed; session created, updated or files-added; event annotation created — deduplicated at an `once_per` grain of occurrence, file, dataset, invocation, session or event, or a five-field UTC cron `Schedule`. Targets are a discriminated union: `InvokeActionTarget` runs an action, `StartAgentTarget` launches an agent, and `SendSlackMessageTarget` posts a templated message. Conditions and target templates read the firing through a variable namespace with a context per entity — `dataset.*`, `file.*`, `invocation.*`, `session.*`, `event.*` (the annotation), plus the reserved `envelope.*` (the platform event's own id, type, time and subject) and `trigger.*`, and the delta contexts `changed.*` and `tag.*` on update events — and entity state is read when the trigger is evaluated rather than captured when the event was emitted. Delivery is at-least-once per target, so a target may run twice for one event; a scheduled trigger skips minutes missed while the scheduler was unavailable rather than running them late. `Trigger.dry_run` returns the same gate-by-gate trace the CLI prints, and `Trigger.dispatches` lists what a trigger fired, with a link to each result.
  - `roboto.uri.RobotoUri` parses and renders the `roboto://<type>/<id>` references that Roboto uses to name an entity without naming a web address — in AI chat answers, notifications, and the `subject` of a platform event. It accepts the optional `?t=` epoch-nanosecond timestamp that time-aware pages honor, exposed as `timestamp_ns`.
